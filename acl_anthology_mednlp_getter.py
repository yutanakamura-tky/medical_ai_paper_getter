...
def mednlp(conference_and_year, verbose=True, toclipboard=False):
    ...
    if conference.lower() not in conferences:
        print(
            "Error: cannot find conference '{}'. \nAvailable conferences are {}."
            .format(conference, ', '.join(conferences)))
    else:
        # get html content
        url = 'https://aclweb.org/anthology/events/{}-{}'.format(
            conference.lower(), str(year))
        print('Connecting...')
...
def mednlp_parse(res, verbose=True, toclipboard=False):
    ...
    # query
    queries = [
        'medic', 'biomedic', 'bioMedic', 'health', 'clinic', 'life', 'care',
        'pharm', 'drug', 'surg', 'emergency', 'ICU', 'hospital', 'patient',
        'doctor', 'disease', 'illness', 'symptom', 'treatment', 'cancer',
        'psycholog', 'psychiat', 'mental', 'radiol', 'x-ray', 'CT', 'MRI',
        'radiograph', 'tomograph', 'magnetic'
    ]
            ...
            for query in queries:
                if not skip:
                    for q in (query, query.upper(), query.capitalize()):
                        if ((('' + q) in title)
                                or title.startswith(q)) and (not skip):
                            link = tag.attrs['href']
                            if link.startswith('/anthology/paper'):
                                result.append(
                                    [title, 'https://aclweb.org' + link])
                                n_match += 1
                                skip = True
                                prev_title = title
                                break
        sys.stdout.write('\rSearching... {} match / {}'.format(
            n_match, n_total))
        sys.stdout.flush()
    sys.stdout.write('\n')
...
if __name__ == '__main__':
    mednlp(
        input("Input conference name and year (e.g. 'naacl 2019') : ").split(),
